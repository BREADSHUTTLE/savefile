...
{
    public class BadugiClient:BaseClient<Packet,Base>
    {
        public BadugiClient(ProtoConnection<Packet> conn, uint protocolVersion) : base(conn, protocolVersion)
        {
        }
        ...
        public async UniTask<PacketResult<ActionRes>> ActionAsync(int table_id, ActionType _aType,long amount,BetSizeType sizeType,CancellationToken ct = default)
        {
        ...
        public async UniTask<PacketResult<DrawRes>> DrawReqAsync(int tableId,List<bool> cardChangeList, CancellationToken ct = default)
        {
            var reqPkt = NewReq(nameof(DrawReq));
            
            reqPkt.DrawReq = new DrawReq() { TableId = tableId};
            reqPkt.DrawReq.DiscardFlags.AddRange(cardChangeList);
            
            var res = await RequestAsync(reqPkt, 3000, ct);
            return new PacketResult<DrawRes>(){Error = res.Base.Error,Data = res.DrawRes};
        }
        
        public async UniTask<PacketResult<KickVoteRes>> KickVoteAsync(int tableId,int targetUid,CancellationToken ct = default)
        {
            var reqPkt = NewReq(nameof(KickVoteReq));
            reqPkt.KickVoteReq = new KickVoteReq() { TableId = tableId,TargetUid = targetUid};
        ...
        public async UniTask<PacketResult<ReadyRes>> ReadyReqAsync(int table_id,bool isReady,CancellationToken ct = default)
        {
            var reqPkt = NewReq(nameof(ReadyReq));
            reqPkt.ReadyReq = new ReadyReq() { TableId = table_id,IsReady = isReady};
            var res = await RequestAsync(reqPkt, 3000, ct);

            return new PacketResult<ReadyRes>(){Error = res.Base.Error,Data = res.ReadyRes};
        }
