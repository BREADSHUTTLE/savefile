...
{
    public class SPokerClient : BaseClient<Packet,Base>
    {
        public SPokerClient(ProtoConnection<Packet> conn, uint protocolVersion) : base(conn, protocolVersion)
        {
        }
        ...
        public async UniTask<PacketResult<CardOpenRes>> CardOpenReqAsync(int table_id,List<string> holecards,CancellationToken ct = default)
        {
            ...
            reqPkt.CardOpenReq.HandCards.AddRange(holecards);
            var res = await RequestAsync(reqPkt, 3000, ct);
        ...
        public async UniTask<PacketResult<SelectRes>> CardSelectReqAsync(int table_id,string drop_card,string open_card ,CancellationToken ct = default)
        {
            var reqPkt = NewReq(nameof(SelectReq));
            reqPkt.SelectReq = new SelectReq() { TableId = table_id,DropCard = drop_card,OpenCard = open_card};
            var res = await RequestAsync(reqPkt, 3000, ct);
            return new PacketResult<SelectRes>(){Error = res.Base.Error,Data = res.SelectRes};
        }
